...
class PresentationApp:
    ...
    def _configure_style(self):
        ...
        style.configure("Header.TLabel", background="#f6f8fc", foreground="#1f2937", font=(font_ui, 26, "bold"))
        style.configure("SubHeader.TLabel", background="#f6f8fc", foreground="#64748b", font=(font_ui, 12))
        style.configure("Title.TLabel", background="#ffffff", foreground="#1f2937", font=(font_ui, 20, "bold"))
        ...
        style.configure("MethodTag.TLabel", background="#ffffff", foreground="#2563eb", font=("Georgia", 13, "bold italic"))
    ...
    def _create_page(self, title, subtitle=None, header_left_image=None, header_right_image=None, content_image=None, image_position="bottom"):
        ...
        if header_left_image is not None:
            ttk.Label(header_row, image=header_left_image, background="#f6f8fc").pack(side="left", padx=(0, 12))

        ttk.Label(header_row, text=title, style="Header.TLabel").pack(side="left")

        if header_right_image is not None:
            ttk.Label(header_row, image=header_right_image, background="#f6f8fc").pack(side="left", padx=(12, 0))
        ...
        accent_bar = tk.Frame(card, bg="#2563eb", height=3)
    ...
    def _nav_buttons(self, nav, page_index):
        ...
        ttk.Label(nav, text=f"Ventana {page_index + 1} de {len(self.pages)}", background="#f6f8fc", foreground="#64748b", font=("Aptos", 10)).pack(side="left", padx=18)
    ...
    def _bullet_list(self, parent, items):
        for item in items:
            ...
            dot = tk.Label(inner, text="•", bg="#ffffff", fg="#2563eb", font=("Aptos", 16, "bold"))
    ...
    def _create_placeholder_image(self, width=250, height=200, label="Imagen"):
        ...
        image.put("#dbeafe", to=(0, 0, width, height))
        
        # Dibujar un borde
        for x in range(width):
            image.put("#2563eb", (x, 0))
            image.put("#2563eb", (x, height - 1))
        for y in range(height):
            image.put("#2563eb", (0, y))
            image.put("#2563eb", (width - 1, y))
    ...
    def _build_pages(self):
        ...
        # 2. Capítulo y tema
        img2 = self._load_logo(["derivadas.png"], max_size=(300, 300))
        if img2:
        ...
        ttk.Label(method_row, text="\nMÉTODOS ABIERTOS", style="MethodTag.TLabel").pack(anchor="center", pady=(0, 12))
        ttk.Label(method_row, text="Requieren un valor inicial que esté cerca de la raíz y que la función sea derivable", style="Body.TLabel").pack(anchor="center", pady=(0, 12))
        ttk.Label(method_row, text="Newton-Raphson", style="MethodTag.TLabel").pack(anchor="center", pady=(0, 12))
        ttk.Label(method_row, text="Utiliza la derivada de la función para aproximar la raíz en cada iteración", style="Body.TLabel", justify="center").pack(anchor="center", pady=(0, 12))
        ttk.Label(method_row, text="Newton-Raphson Mejorado", style="MethodTag.TLabel").pack(anchor="center", pady=(0, 12))
        ttk.Label(method_row, text="Incorpora una segunda derivada para mejorar la convergencia en casos difíciles", style="Body.TLabel", justify="center").pack(anchor="center", pady=(0, 12))
        self.pages.append((page, nav))

        # 3. Tema y utilidad
        img3 = self._load_logo(["derivada2.png"], max_size=(200, 200))
        if img3:
            self.images.append(img3)
        page, card, nav = self._create_page("Utilidad", "¿Para qué sirve?", content_image=img3, image_position="bottom")
        self._bullet_list(card, [
            ...
            "Cuando se tiene una buena estimación inicial ahorra iteraciones y tiempo de cálculo.\n",
            "Es útil donde aparecen modelos no lineales.",
        ])
        self.pages.append((page, nav))

        # 4. Aplicaciones
        img4 = self._load_logo(["aplicaciones.png"], max_size=(300, 300))
        if img4:
            self.images.append(img4)
        page, card, nav = self._create_page("Aplicaciones", "Dónde se usan estos métodos", content_image=img4, image_position="bottom")
        self._bullet_list(card, [
            "Matemáticas: Encontrar raíces de polinomios.",
            "Física: Calcular tiempos de caída y resolver problemas de movimiento.",
            "Ingeniería: Análisis de circuitos eléctricos",
            "Economía: Encontrar puntos de equilibrio en modelos económicos.",
            "Ciencias de la computación: Optimización y aprendizaje automático.",
            ])
        self.pages.append((page, nav))
