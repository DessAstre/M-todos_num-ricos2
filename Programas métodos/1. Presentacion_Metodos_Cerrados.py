...
class PresentationApp:
    ...
    def _build_pages(self):
        ...
        page, card, nav= self._create_page("Método de Bisección del Intervalo", "Paso a paso")
        self._bullet_list(card, [
            "\n1. Verificar que f(a) y f(b) tengan signos opuestos\n",
            "2. Si f(a)=0, entonces a es la raíz. Si f(b)=0, entonces b es la raíz\n",
            "3. Calcular el punto medio c = (a + b) / 2\n",
            "4. Se calcula la nueva aproximación x = a + c\n",
            "5. Si |c| ≤ tolerancia, x es la raíz aproximada\n",
            "6. Evaluar f(x)\n",
            "7. Si f(x)=0, entonces x es la raíz exacta\n",
            "8. Si f(x) tiene el mismo signo que f(a), entonces a = x; de lo contrario, b = x\n",
            "9. Repetir desde el paso 2 hasta cumplir la condición de tolerancia"
        ])
        self.pages.append((page, nav))
        
        page, card, nav=self._create_page("Ventajas y desventajas")
        self._bullet_list(card, [
            "\nVentajas de Bisección:\n",
            "✓ Garantiza convergencia a una raíz si la función es continua y cambia de signo en el intervalo\n",
            "✓ Fácil de implementar y entender\n",
            "✓ No requiere cálculo de derivadas\n",
            "\nDesventajas de Bisección:\n",
            "✖ Convergencia lenta, especialmente para funciones con raíces múltiples o cercanas\n",
            "✖ Requiere que se conozca un intervalo inicial con cambio de signo\n",
            "✖ No es eficiente para funciones con comportamiento oscilatorio o raíces muy cercanas"
        ])

        page, card, nav = self._create_page("Método de la Falsa Posición", "Paso a paso")
        self._bullet_list(card, [
            "1. Verificar que f(a) y f(b) tengan signos opuestos",
            "2. Si f(a)=0, entonces a es la raíz. Si f(b)=0, entonces b es la raíz",
            "3. Calcular Δx = |f(a)|(b-a)/(|f(a)|-|f(b)|)",
            "4. Calcular la nueva aproximación x = a + Δx",
            "5. Evaluar f(x)",
            "6. Si |f(x)| ≤ tolerancia, x es la raíz aproximada",
            "7. Si f(x) tiene el mismo signo que f(a), entonces a = x; de lo contrario, b = x",
            "8. Repetir desde el paso 2 hasta cumplir la condición de tolerancia"
        ])
        self.pages.append((page, nav))

        page, card, nav = self._create_page("Ventajas y desventajas")
        self._bullet_list(card, [
            "\nVentajas de Falsa Posición:\n",
            "✓ Convergencia más rápida que Bisección para funciones con raíces simples\n",
            "✓ Garantiza convergencia a una raíz si la función es continua y cambia de signo en el intervalo\n",
            "✓ No requiere cálculo de derivadas\n",
            "\nDesventajas de Falsa Posición:\n",
            "✖ Puede sufrir de convergencia lenta o estancamiento para funciones con raíces múltiples o cercanas\n",
            "✖ Requiere que se conozca un intervalo inicial con cambio de signo\n",
            "✖ No es eficiente para funciones con comportamiento oscilatorio o raíces muy cercanas"
        ])
